...
int main()
{
	...
	#pragma omp parallel num_threads(5) shared(sum) private(k)
	{
		...
		// В цикле считаем площадь каждого отрезка. Используем редукцию для переменной sum. 
		// Получаем 10-ти кратное ускорение (при данных параметрах) на моем компьютере
